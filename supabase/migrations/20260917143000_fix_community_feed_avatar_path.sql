-- user_profiles.avatar_url n'existe plus sur prod : chemin avatar depuis photos jsonb uniquement

create or replace function public.user_profile_avatar_storage_path(p_photos jsonb)
returns text
language sql
immutable
...
as $$
  select coalesce(
    nullif(trim(p_photos->>'profile_photo_path'), ''),
    nullif(trim(p_photos->>'profilePhotoPath'), ''),
    nullif(trim(p_photos->'profile'->>'storage_path'), ''),
    nullif(trim(p_photos->'profile'->>'storagePath'), ''),
    nullif(trim(p_photos->'profile'->>'path'), ''),
    nullif(trim(p_photos->>'profile'), '')
  );
$$;
