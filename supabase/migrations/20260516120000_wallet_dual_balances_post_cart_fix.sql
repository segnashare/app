-- Double solde (consommation / échange) + credit_bucket sur wallet_transactions.
-- Placée après 20260515103000 pour ne pas être écrasée par l’ancien débit panier mono-solde.
-- L’ancienne 20260410210000 a été retirée : sur `db reset`, l’ordre des migrations reste cohérent.
-- Trigger INSERT : si consommation et échange restent à 0 mais balance_points > 0 (insert legacy),
--   on pousse le total sur balance_exchange_points puis on recalcule balance_points.
...
comment on column public.user_wallets.balance_consumption_points is
  'Crédits de consommation (récompenses, achats type invité, compléments consommation).';
comment on column public.user_wallets.balance_exchange_points is
  'Crédits d''échange (prêts validés, compléments échange abonné).';
...
alter table public.wallet_transactions
  ...
  check (credit_bucket is null or credit_bucket in ('consumption', 'exchange', 'mixed'));

comment on column public.wallet_transactions.credit_bucket is
  'Type de crédit concerné : crédit simple, ou débit ventilé via metadata.debit_split.';

-- Init split : tout l’historique existant sur le compte échange (approximation ; ajustement manuel possible).
...
-- Rétro-étiquetage des crédits connus
update public.wallet_transactions wt
set credit_bucket = 'exchange'
where wt.credit_bucket is null
  and wt.direction = 'credit'
  and coalesce(wt.metadata->>'source', '') = 'lend_intake_verified';

update public.wallet_transactions wt
set credit_bucket = 'consumption'
where wt.credit_bucket is null
  and wt.direction = 'credit'
  and coalesce(wt.metadata->>'source', '') = 'credits_purchase'
  and lower(trim(coalesce(wt.metadata->>'credits_kind', ''))) in ('consumption', 'pods', 'consommation');
...
where wt.credit_bucket is null
  and wt.direction = 'credit'
  and coalesce(wt.metadata->>'source', '') = 'credits_purchase'
  and lower(trim(coalesce(wt.metadata->>'credits_kind', ''))) in ('exchange', 'mods');
...
begin
  ...
  update public.user_wallets uw
     set
       ...
       balance_exchange_points = uw.balance_exchange_points
         + case when v_effective_kind = 'exchange' then p_amount_points else 0 end,
       updated_at = now()
   where uw.id = (
      select id
...
-- ---------------------------------------------------------------------------
-- Crédit prêt (intake vérifié) → compte échange uniquement
-- ---------------------------------------------------------------------------
...
begin
  ...
  update public.user_wallets uw
     set
       balance_exchange_points = uw.balance_exchange_points + v_points,
       updated_at = now()
   where uw.id = (
      select id
      from public.user_wallets
...
-- ---------------------------------------------------------------------------
-- Débit panier : ventile complément Stripe (metadata credits_kind) + reste (abonné → échange, invité → consommation)
-- ---------------------------------------------------------------------------
...
begin
  ...
  if v_ex_bal < v_debit_ex or v_co_bal < v_debit_co then
  ...
  update public.user_wallets uw
  set
    balance_exchange_points = uw.balance_exchange_points - v_debit_ex,
...
comment on function public.wallet_debit_cart_order_stripe(uuid, uuid, text, text, jsonb) is
  'Débite le panier en ventilant échange / consommation (metadata debit_split + credit_bucket).';

-- ---------------------------------------------------------------------------
-- Retrait pièce retired → débit compte échange
-- ---------------------------------------------------------------------------
...
begin
  ...
  update public.user_wallets uw
     set
       balance_exchange_points = greatest(0::bigint, uw.balance_exchange_points - v_points),
       updated_at = now()
   where uw.id = (
      select id
      from public.user_wallets
