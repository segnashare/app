-- Expose dispute_id + dispute_created dans le retour RPC pour déclencher n8n côté app.
...
declare
  ...
  v_ret_status text;
  v_dispute_id uuid;
  v_dispute_created boolean := false;
...
begin
  ...
  select o.id, o.formal_notice_deadline_at
    into v_overdue_id, v_formal_notice_deadline
  ...
  if v_formal_notice_deadline is not null
    ...
    return jsonb_build_object(
      ...
      'dispute_id', case when v_dispute_created then v_dispute_id else null end,
      'dispute_created', v_dispute_created
    );
  end if;
  ...
  select coalesce(o.penalties_accrued_cents, 0)
    into v_penalties_accrued
  from public.cart_borrow_overdue o
...
comment on function public.accrue_cart_borrow_overdue_day(uuid, date, boolean) is
  'Pénalités 3 %/jour sans plafond ; escalade J+15 ; retourne dispute_created pour webhook n8n.';
