-- La modale d'accueil in-app (`(main)/layout.tsx`) s'affiche si `users.onboarding_process = 'intro'`.
-- `complete_onboarding` mettait à jour `status` / `onboarding_completed_at` sans ce champ.

alter table public.users
  add column if not exists onboarding_process text null;

comment on column public.users.onboarding_process is
  'Parcours in-app après onboarding accueil (intro → profile → kyc → …).';
...
begin
  ...
  set status = 'active',
      ...
      onboarding_process = coalesce(onboarding_process, 'intro')
  where id = v_uid;
