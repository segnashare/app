-- Pénalités retard : plus de débit wallet — statut pending, prélèvement carte côté app (Stripe).
...
comment on function public.accrue_cart_borrow_overdue_day(uuid, date, boolean) is
  'Journalise 1 jour de retard ; prélèvement EUR via Stripe (app), sans débit wallet.';

comment on column public.cart_borrow_overdue_days.stripe_payment_intent_id is
  'PaymentIntent Stripe (prélèvement carte en EUR).';
