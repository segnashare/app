-- member_intake → dropped_out : item_intake validated + ready → shipping (comme dropped_in).
...
comment on function public.transition_shipment_status(
  ...
  'Statut colis ; member_intake dropped_out → intake shipping ; delivered/returned/en_verification → in_verification.';

-- Rattrapage : colis déjà dropped_out, pièces encore ready.
select public.promote_member_intake_items_to_shipping(s.id)
from public.shipments s
where s.deleted_at is null
  and s.context = 'member_intake'::public.shipment_context
  and lower(s.status::text) = 'dropped_out';
