-- Annulation immédiate : solde wallet (conso + échange) passé à zéro, avec écritures d’audit.
...
comment on function public.wallet_forfeit_on_immediate_subscription_cancel(uuid, text, text) is
  'Remet le wallet à zéro à l’annulation immédiate d’abonnement (idempotent par subscription_id).';
