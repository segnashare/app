-- Annulation membre : expédition aller `pending` ou `ready` (avant prise en charge transporteur).
...
comment on function public.member_cancel_cart_order_pending_preparation(uuid) is
  'Membre : annule une commande confirmée tant que l’expédition aller est pending ou ready. Rembourse crédits (débits panier agrégés), items → listed, panier canceled. Remboursement € Stripe : route API avant appel.';
