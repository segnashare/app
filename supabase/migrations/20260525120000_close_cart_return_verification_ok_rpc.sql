-- Clôture retour (voir 20260527130000_return_post_clean_item_cleaning_drop_cart_needs_cleaning.sql pour post_clean / item cleaning).
-- Recrédit wallet (même idempotency que le backoffice), panier → archived, lignes → archived, expédition retour → closed.
-- Ne traite pas les litiges (lignes rejected) : utiliser le flux BO disputed + finalisation existant.
...
begin
  ...
  -- Panier déjà archivé : ne fermer l’expédition que si besoin (idempotence / réparation).
  ...
  -- Crédit wallet (même logique que l’API cart-return-close).
