-- Suggestions tenue panier : ne garder que les pièces dont la taille correspond au profil membre.
...
as $$
  select
    p_item_size_id is not null
    and exists (
      select 1
      from public.user_profiles up
      join public.user_profile_sizes upsz on upsz.user_profile_id = up.id
      join public.sizes sz_item on sz_item.id = p_item_size_id
      where up.user_id = p_user_id
        and upsz.size_id = p_item_size_id
        and upsz.category = case
          when sz_item.code like 'top:%' then 'top'
          when sz_item.code like 'bottom:%' then 'bottom'
          when sz_item.code like 'shoes:%' then 'shoes'
          else null
        end
    );
$$;

grant execute on function public.item_size_matches_member_profile(uuid, uuid) to authenticated;

comment on function public.item_size_matches_member_profile(uuid, uuid) is
  'Vrai si la taille pièce correspond à une taille profil membre (haut / bas / chaussures).';
...
begin
  ...
  where l.published = true
    ...
    and public.item_size_matches_member_profile(ci.item_size_id, v_uid)
    and not exists (
      select 1
      from public.users u
...
comment on function public.get_cart_outfit_suggestions(uuid[], uuid[], integer) is
  'Suggestions upsell panier : companions CMS + fallback complémentaires, taille membre + disponibles uniquement.';
