-- Crédits inclus Guest : activation unique à l’onboarding (app), pas de grant wallet mensuel.
-- `billing_upsert_monthly_entitlement` conserve la ligne `user_monthly_entitlements` pour les quotas
-- (ex. échanges gratuits / commandes incluses), sans créditer le wallet pour `guest`.
...
comment on function public.billing_upsert_monthly_entitlement(uuid, text, date) is
  'Upsert ligne mensuelle (quotas) ; crédit wallet consommation uniquement Segna+ / SegnaX. Guest : quotas seuls, crédits inclus = activation onboarding one-shot.';
