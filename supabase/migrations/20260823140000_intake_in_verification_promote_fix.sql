-- Fix : member_intake en returned / en_verification sans passage item_intake → in_verification
-- (lien merge, transition en_verification seule, ou métadonnées incomplètes).
  ...
  if v_context = 'member_intake' then
    ...
    for v_row in
      ...
      where ii.item_id = any (v_ids)
         or ii.metadata->'sendcloud'->>'sc_member_intake_shipment_id' = v_sid
         or ii.metadata->'sendcloud'->>'sc_dummy_shipment_id' = v_sid
    loop
      if coalesce(v_row.metadata->'sendcloud'->>'sc_merge_item_ids', '') <> '' then
        foreach v_part in array string_to_array(v_row.metadata->'sendcloud'->>'sc_merge_item_ids', ',') loop
          v_part := btrim(v_part);
...
comment on function public.promote_intake_items_to_in_verification_on_shipment_delivered(uuid) is
  'Réception colis (delivered/returned/en_verification) : intake validated en shipping → in_verification.';

-- transition_shipment_status : aussi au passage en en_verification (file BO).
...
-- Rattrapage : colis member_intake / cart_return reçus ou en contrôle, pièces encore shipping.
...
where s.deleted_at is null
  ...
  and lower(s.status::text) in ('delivered', 'returned', 'en_verification');
