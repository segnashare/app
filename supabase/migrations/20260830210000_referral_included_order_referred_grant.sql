-- Parrainage : +1 échange inclus pour la filleule en plus de la marraine (déjà créditée).
-- Backfill des parrainages qualifiés avant cette migration.
...
begin
  ...
  begin
    ...
    perform public.grant_user_included_order_credit(
      v_uid,
      1,
      'referral_included_order:' || v_referral_id::text || ':referred',
      jsonb_build_object('source', 'referral_signup', 'referral_id', v_referral_id, 'role', 'referred')
    );
...
comment on function public.qualify_pending_referral(uuid) is
  'Qualifie un filleul (tél. vérifié + onboarding) : +1 échange inclus filleule et marraine.';

-- Parrainages déjà qualifiés sans grant filleule / marraine (migration 30200000).
select public.grant_user_included_order_credit(
  r.referred_user_id,
  1,
  'referral_included_order:' || r.id::text || ':referred',
  jsonb_build_object('source', 'referral_signup_backfill', 'referral_id', r.id, 'role', 'referred')
)
...
select public.grant_user_included_order_credit(
  r.referrer_user_id,
  1,
  'referral_included_order:' || r.id::text || ':referrer',
  jsonb_build_object('source', 'referral_signup_backfill', 'referral_id', r.id, 'role', 'referrer')
)
from public.referrals r
where r.status = 'qualified'
