-- Suggestions panier : tenues Style (style_looks) + tenues par pièce (item_outfit_looks), sans filtre taille.
...
begin
  ...
  with
  -- Companions des tenues par pièce (ancre = pièce panier).
  ...
  -- Companions des tenues Style (toute pièce du panier présente dans la tenue).
...
comment on function public.get_cart_outfit_suggestions(uuid[], uuid[], integer) is
  'Suggestions panier : companions tenues Style + tenues par pièce, statut available, sans filtre taille.';
