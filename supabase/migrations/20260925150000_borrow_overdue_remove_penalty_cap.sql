-- Suppression du plafond 100 % sur les frais de retard (accrue illimité jusqu'à MED / retour).
...
comment on function public.borrow_overdue_penalty_cap_cents(bigint) is
  'Déprécié : plus de plafond contractuel sur les frais de retard.';
...
comment on function public.accrue_cart_borrow_overdue_day(uuid, date, boolean) is
  'Pénalités 3 %/jour sans plafond ; escalade J+15 ; stop accrue après formal_notice_deadline_at.';

update public.cart_borrow_overdue
set penalty_cap_cents = null
where penalty_cap_cents is not null;
