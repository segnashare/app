-- Après paiement Stripe (commande panier) : panier → confirmed, expédition aller + lignes, fin des verrous checkout.
...
comment on function public.confirm_cart_paid_from_stripe(uuid, uuid, text, text, text, text) is
  'Payé sur Stripe : panier checkout_pending|active → confirmed, shipment cart_outbound + items + destination. Idempotent si déjà confirmed.';

revoke all on function public.confirm_cart_paid_from_stripe(uuid, uuid, text, text, text, text) from public;
grant execute on function public.confirm_cart_paid_from_stripe(uuid, uuid, text, text, text, text) to service_role;
