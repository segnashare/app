-- Post-retour pressing : statut catalogue `cleaning` + flag JSON `cart_items.return_verification.post_clean`
-- (plus de valeur enum `needs_cleaning` sur les lignes panier).
-- S'exécute après guest_exchange_wallet_guard pour remplacer close_cart_return_verification_ok.

-- 1) item_status.cleaning (après retired, avant archived)
do $$
begin
  if not exists (
    select 1
    ...
    alter type public.item_status add value 'cleaning' after 'retired';
  end if;
end $$;

comment on type public.item_status is
  'Statut operational item. listed=catalogue sans emprunt immediat ; available=panier/reservation possibles ; cleaning=pressing logistique post-retour.';

-- 2) Retrait needs_cleaning sur cart_item_status (données -> verified)
update public.cart_items ci
set
  status = 'verified'::public.cart_item_status,
  updated_at = timezone('utc', now())
where ci.deleted_at is null
  and ci.status = 'needs_cleaning'::public.cart_item_status;

-- Supabase PG actuel : pas de DROP VALUE sur enum (non supporté). La valeur needs_cleaning peut rester
-- dans le type tant qu’aucune ligne ne l’utilise (migrées en verified ci-dessus).

comment on type public.cart_item_status is
  'Statut ligne panier : réservation (in_cart, reservation_pending, reserved), cycle retour (verification_pending, verified, rejected), archived. Valeur enum legacy needs_cleaning inutilisée.';

-- 3) close_cart_return_verification_ok : post_clean / item cleaning + lignes deja archivees (sale)
...
comment on function public.close_cart_return_verification_ok(uuid, uuid) is
  'Retour BO : crédit wallet idempotent, panier archived, expédition closed. Bloque si pressing post_clean ou item cleaning ; cloture possible quand toutes les lignes sont archivees (sale). service_role uniquement.';
