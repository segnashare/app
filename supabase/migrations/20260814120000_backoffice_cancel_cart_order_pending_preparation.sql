-- Annulation back-office : commande confirmée tant que l’expédition aller est **pending** uniquement
-- (équivalent « En préparation » dans le BO). Même logique métier que l’annulation membre mais
-- réservée au rôle `service_role` (pas de auth.uid() membre).
...
comment on function public.backoffice_cancel_cart_order_pending_preparation(uuid, uuid) is
  $cmt$Back-office (service_role) : annule une commande confirmée tant que l'expédition aller est pending. Remboursement € Stripe : route API segna-app avant appel.$cmt$;
