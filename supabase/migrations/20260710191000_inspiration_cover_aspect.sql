-- Looks / inspis : formats de couverture Pinterest (portrait, paysage, carré) + cadrage.

alter table public.community_inspirations
  add column if not exists cover_aspect text not null default 'portrait'
    check (cover_aspect in ('landscape', 'portrait', 'square')),
...
comment on column public.community_inspirations.cover_aspect is
  'Format d''affichage feed : landscape (4:3), portrait (3:4), square (1:1).';
comment on column public.community_inspirations.cover_transform is
  'Cadrage couverture JSON { offset: { x, y }, zoom } en pourcentage.';

drop function if exists public.publish_community_inspiration(
  uuid, text, text, public.inspiration_media_type, text, jsonb, text, uuid[], text[]
);
...
-- Feed : expose cover_aspect / cover_transform dans les cartes.
...
as $$
  ...
  from (
    ...
    where sl.published = true
      and sl.id <> p_look_id
    order by sl.published_at desc nulls last
    limit 12
  ) ranked;
$$;

-- Détail inspi : cover_aspect / cover_transform
...
begin
  ...
  if p_source = 'segna_style'::public.inspiration_source then
    ...
    group by ci.id, up.display_name, up.photos;
