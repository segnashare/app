-- Distingue looks Segna (membre / @segna) et inspis externes (crédit Instagram, sans membre).

alter table public.style_looks
  add column if not exists entry_kind text not null default 'look';

alter table public.style_looks
  add column if not exists source_instagram_username text null;

update public.style_looks
set entry_kind = 'look'
where entry_kind is null or entry_kind not in ('look', 'inspi');

alter table public.style_looks
  drop constraint if exists style_looks_entry_kind_check;

alter table public.style_looks
  add constraint style_looks_entry_kind_check
  check (entry_kind in ('look', 'inspi'));

alter table public.style_looks
  drop constraint if exists style_looks_inspi_no_member_check;

alter table public.style_looks
  add constraint style_looks_inspi_no_member_check
  check (entry_kind <> 'inspi' or featured_member_user_id is null);

comment on column public.style_looks.entry_kind is
  'look = comment porter les pièces Segna (membre ou @segna). inspi = look externe lié à des pièces semblables, sans membre.';

comment on column public.style_looks.source_instagram_username is
  'Compte Instagram source d''une inspi (handle sans @). Null pour les looks.';
...
begin
  ...
  segna_rows as (
    select
      ...
      sl.entry_kind,
      case
        when sl.entry_kind = 'inspi' then null::uuid
        else sl.featured_member_user_id
      end as author_user_id,
      case
        when sl.entry_kind = 'inspi' then coalesce(nullif(trim(sl.source_instagram_username), ''), 'Inspi')
        when sl.featured_member_user_id is null then 'Segna'::text
        else coalesce(nullif(trim(up_feat.display_name), ''), 'Membre Segna')
      end as author_display_name,
      case
        when sl.entry_kind = 'inspi' then null::text
      ...
      case
        when sl.entry_kind = 'inspi' then nullif(trim(sl.source_instagram_username), '')
        when sl.featured_member_user_id is null then null::text
    ...
    select
      jsonb_build_object(
        ...
        'is_following_author', case
          when sl.entry_kind = 'inspi' then false
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'entry_kind', ranked.entry_kind,
        'author_display_name', case
          when ranked.entry_kind = 'inspi' then coalesce(nullif(trim(ranked.source_instagram_username), ''), 'Inspi')
          when ranked.featured_member_user_id is null then 'Segna'::text
          else coalesce(nullif(trim(ranked.display_name), ''), 'Membre Segna')
        end,
        'author_instagram_username', case
          when ranked.entry_kind = 'inspi' then nullif(trim(ranked.source_instagram_username), '')
