-- Upsell checkout « Terminez votre commande » : catalogue taille membre, disponible, 10 max (≠ tenues CMS).
...
grant execute on function public.get_cart_upsell_suggestions(uuid[], uuid[], integer) to authenticated;

comment on function public.get_cart_upsell_suggestions(uuid[], uuid[], integer) is
  'Upsell checkout : pièces available à la taille membre, hors départements du panier, max 10.';

comment on function public.get_cart_outfit_suggestions(uuid[], uuid[], integer) is
  'Panier « Complétez votre tenue » : companions tenues Style + tenues par pièce, sans filtre taille.';
