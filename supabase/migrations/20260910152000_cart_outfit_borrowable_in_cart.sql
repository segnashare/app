-- Empruntable pour suggestions tenue : aligné catalogue shop (available | in_cart), sans filtre taille.
...
comment on function public.item_borrowable_for_outfit_suggestion(uuid, uuid) is
  'Pièce éligible rail « Complétez votre tenue » (available ou in_cart, hors stock perso / corporate / phantom).';

grant execute on function public.item_borrowable_for_outfit_suggestion(uuid, uuid) to authenticated;
...
begin
  ...
  eligible as (
    ...
    where public.item_borrowable_for_outfit_suggestion(mc.id, v_uid)
  ),
  ranked as (
    select
      id,
...
comment on function public.get_cart_outfit_suggestions(uuid[], uuid[], integer) is
  'Suggestions panier : tenues Style + tenues par pièce, pièces available/in_cart, sans filtre taille.';
