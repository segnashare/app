-- Réparation : 20260830180000 peut être déjà présente dans schema_migrations sans que la fonction
-- ait été remplacée. Ce fichier est idempotent (CREATE OR REPLACE).
...
begin
  ...
  if p_plan_code in ('guest', 'segna_plus', 'segna_x') then
...
comment on function public.billing_upsert_monthly_entitlement(uuid, text, date) is
  'Upsert ligne mensuelle + crédit wallet consommation (Guest / Segna+ / SegnaX) idempotent par mois/plan ; montant = billing_plan_limits.monthly_consumption_points_grant.';
