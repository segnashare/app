-- Soft-delete membre self-serve :
-- - anonymise le profil, masque looks / médias community
-- - purge storage profile/looks/inspirations
-- - conserve carts / payments / shipments (historique Segna)
-- - ne touche pas aux photos catalogue (bucket_items)
...
begin
  ...
  -- Soft-delete + anonymisation PII (garde l’UUID pour l’historique commandes).
  ...
  -- Médias storage : purgés via Storage API côté route Next (pas de DELETE storage.objects).
...
comment on function public.request_my_account_deletion(text) is
  'Soft-delete self-serve : anonymise + masque médias/looks, conserve l’historique commandes. Bloqué si locations ouvertes.';
