-- Échange, réservation panier et crédits d’échange : ouverts à tout membre identifié,
-- sans condition d’abonnement Membre + / Membre X.
-- (Les avantages forfait abonnement — livraisons incluses, plafonds prêt — restent côté app / entitlements.)

-- ---------------------------------------------------------------------------
-- Réservation inventaire + garde-fous wallet (triggers / RPC alignés sur cette fonction)
-- ---------------------------------------------------------------------------
...
comment on function public.user_can_reserve_cart_inventory(uuid) is
  'True si un utilisateur cible est identifié : réservation panier, crédits d’échange et prêts sans exiger un abonnement prêteur.';

-- ---------------------------------------------------------------------------
-- Nouveau portefeuille : solde échange matérialisé (0) pour tout le monde
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- Données : anciens portefeuilles sans colonne échange matérialisée
-- ---------------------------------------------------------------------------
...
where uw.deleted_at is null
  and uw.balance_exchange_points is null;

-- Intakes historiques bloqués sur l’ancienne règle « abonnement requis »
