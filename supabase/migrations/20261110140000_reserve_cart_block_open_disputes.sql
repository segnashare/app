-- Bloque reserve_cart_atomic si le membre a un litige panier ou pièce encore ouvert.
...
begin
  ...
  if exists (
    ...
    join public.carts c on c.id = cd.cart_id
    where c.user_id = v_uid
      and cd.deleted_at is null
      and cd.status in ('open', 'in_review')
  ) or exists (
    select 1
    from public.item_disputes id
    join public.cart_disputes cd on cd.id = id.cart_dispute_id
    join public.carts c on c.id = cd.cart_id
    where c.user_id = v_uid
      and cd.deleted_at is null
      and id.deleted_at is null
      and id.status in ('open', 'in_review')
  ) then
    raise exception 'MEMBER_OPEN_DISPUTE_BLOCKS_RESERVE';
  end if;
