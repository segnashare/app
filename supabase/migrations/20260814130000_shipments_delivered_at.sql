-- Date de livraison effectuelle (aller) : figée au passage en `delivered`, base du calcul des délais d'emprunt.
-- `updated_at` peut encore bouger (ex. mise à jour lien suivi) sans décaler l'échéance.

alter table public.shipments
  add column if not exists delivered_at timestamptz;

comment on column public.shipments.delivered_at is
  'Réception aller : renseigné une fois au passage en status delivered (transition ou backfill). Référence des délais d''emprunt.';

update public.shipments s
set delivered_at = s.updated_at
...
comment on function public.transition_shipment_status(
  ...
  'Met à jour shipments.status (verrou optimistic sur p_if_current_status), ready_at si passage en ready, delivered_at si premier passage en delivered, tracking optionnel, puis append_shipment_status_history. service_role.';
