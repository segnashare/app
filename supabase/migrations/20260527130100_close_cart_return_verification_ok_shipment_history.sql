-- Journalise la fermeture d’expédition retour dans shipment_status_history (RPC append).
-- À appliquer après 20260527130000_return_post_clean_item_cleaning_drop_cart_needs_cleaning.sql.
...
comment on function public.close_cart_return_verification_ok(uuid, uuid) is
  'Retour BO : crédit wallet idempotent, panier archived, expédition closed + historique shipment. Bloque si pressing post_clean ou item cleaning ; cloture possible quand toutes les lignes sont archivees (sale). service_role uniquement.';
