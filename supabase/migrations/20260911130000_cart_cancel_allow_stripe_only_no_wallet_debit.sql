-- Annulation panier : autoriser les commandes payées 100 % en € Stripe (pas de débit wallet).
-- Ces paniers n'ont pas de wallet_transactions source=cart_order_stripe, seulement cart_payments / facture.
...
begin
  ...
  if v_debit_anchor_id is null or (v_ex <= 0 and v_co <= 0) then
    if not exists (
      select 1 from public.cart_payments cp where cp.cart_id = p_cart_id
    )
    and not exists (
      select 1 from public.cart_order_stripe_invoices inv where inv.cart_id = p_cart_id
    ) then
  ...
  return jsonb_build_object(
    ...
    'wallet_debit_skipped', (v_debit_anchor_id is null)
  );
end;
$fn$;

comment on function public.member_cancel_cart_order_pending_preparation(uuid) is
  'Membre : annule une commande confirmée tant que l''expédition aller est pending ou ready. '
  'Rembourse crédits wallet si débités ; accepte aussi paiement 100 % Stripe € sans débit wallet. '
  'Remboursement € Stripe : route API avant/après selon flux.';
...
comment on function public.backoffice_cancel_cart_order_pending_preparation(uuid, uuid) is
  'Back-office (service_role) : annule une commande confirmée tant que l''expédition aller est pending. '
  'Rembourse crédits wallet si débités ; accepte aussi paiement 100 % Stripe € sans débit wallet. '
  'Remboursement € Stripe : route API segna-app.';
