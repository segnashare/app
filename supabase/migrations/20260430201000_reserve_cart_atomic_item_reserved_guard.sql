-- Refuse la réservation si une pièce du sous-ensemble est déjà items.status = reserved (autre membre).
...
begin
  ...
  if exists (
    ...
    where wt.idempotency_key = v_idem
      and wt.kind = 'hold'
      and wt.status = 'posted'
  ) then
    return jsonb_build_object('ok', true, 'idempotent', true, 'cart_id', p_cart_id);
  end if;
