-- Crédits wallet : libellés canoniques consumption | exchange (legacy pods | mods encore acceptés en entrée).
...
set metadata = jsonb_set(
  ...
  array['credits_kind']::text[],
  to_jsonb(
    case lower(trim(coalesce(wt.metadata->>'credits_kind', '')))
      when 'pods' then 'consumption'
      when 'mods' then 'exchange'
      else wt.metadata->>'credits_kind'
    end
  ),
  true
)
where wt.metadata ? 'credits_kind'
  and lower(trim(coalesce(wt.metadata->>'credits_kind', ''))) in ('pods', 'mods');
