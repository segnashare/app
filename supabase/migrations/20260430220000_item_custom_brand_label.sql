-- Libellé marque libre lorsque item_brand_id pointe sur la marque « Autre » (slug autre).
-- Après get_shop_catalog_items (202604302045) pour conserver les définitions RPC à jour.

alter table public.items
  add column if not exists item_custom_brand_label varchar(30);

comment on column public.items.item_custom_brand_label is
  'Texte affiché à la place du libellé item_brands quand la marque choisie est « Autre » (non persisté dans item_brands).';

insert into public.item_brands (label, slug)
select 'Autre', 'autre'
where not exists (
  select 1 from public.item_brands ib where ib.slug = 'autre'
);

-- Shop catalogue : brand_label affiche le libellé personnalisé si renseigné.
...
begin
  ...
  from (
    select
      ...
      coalesce(nullif(trim(i.item_custom_brand_label), ''), br.label) as brand_label,
      (
