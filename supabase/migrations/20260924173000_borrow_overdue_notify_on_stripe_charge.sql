-- `notified_at` sur `cart_borrow_overdue_days` = outreach envoyé (e-mail), pas marqué à l'accrue.
-- L'envoi est déclenché après PaymentIntent Stripe réussi (`notifyBorrowOverdueAfterStripeCharge`).
...
update public.cart_borrow_overdue_days d
set notified_at = null
where d.charge_status = 'charged'
  and d.stripe_payment_intent_id is not null
  and d.notified_at is not null
  and not exists (
    select 1
    from public.notification_send_log n
    where n.idempotency_key = 'txn:lc:borrow_overdue_stripe:' || d.stripe_payment_intent_id
  );
...
comment on function public.accrue_cart_borrow_overdue_day(uuid, date, boolean) is
  'Journalise 1 jour de retard. notified_at = outreach post-prélèvement Stripe (p_force_notify conservé, no-op).';
