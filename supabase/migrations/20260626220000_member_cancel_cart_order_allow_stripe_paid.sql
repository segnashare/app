-- Annulation membre : le remboursement carte est orchestré côté API (Stripe) avant cet RPC ;
-- on retire le bloc qui interdisait toute annulation dès qu’une facture panier € existait.
...
comment on function public.member_cancel_cart_order_pending_preparation(uuid) is
  'Membre : annule une commande confirmée tant que l’expédition aller est pending. Rembourse crédits (débits panier agrégés), items → listed, panier canceled. Remboursement € Stripe : route API avant appel.';
