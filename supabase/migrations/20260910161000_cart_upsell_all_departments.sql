-- Upsell : tout le catalogue éligible (pas seulement les départements « complémentaires »).
-- Si le panier contient déjà des vêtements, on peut quand même proposer d'autres vêtements < 50 cr.
...
comment on function public.get_cart_upsell_suggestions(uuid[], uuid[], integer) is
  'Upsell checkout : catalogue (tous départements), available, taille membre, < 50 crédits, ordre aléatoire, max 10.';
