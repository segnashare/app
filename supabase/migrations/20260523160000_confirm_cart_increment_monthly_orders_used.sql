-- À chaque panier confirmé (Stripe) : +1 sur `user_monthly_entitlements.orders_used` pour abonnés actifs,
-- afin d’aligner le quota `remaining_orders_this_month` avec les livraisons réellement « consommées ».
...
begin
  ...
  -- Quota mensuel « commandes / livraisons incluses » (Membre + / X).
...
comment on function public.confirm_cart_paid_from_stripe(uuid, uuid, text, text, text, text) is
  'Payé sur Stripe : panier checkout_pending|active → confirmed, lignes pré-expédition + items → reserved, shipment cart_outbound. Incrémente orders_used (abonnés actifs). Idempotent ; réparation reserved si déjà confirmed.';
