...
import { EmpruntBorrowDelayLearnMore } from "@/components/emprunt/EmpruntBorrowDelayLearnMore";
...
const btnPrimary = cn(
  ...
  "inline-flex w-full min-w-0 items-center justify-center rounded-full bg-black px-4 py-2.5 text-center text-[15px] font-bold leading-none text-white transition hover:bg-zinc-900 active:scale-[0.99] sm:px-5 sm:py-3 sm:text-[16px]",
);
        ...
        <div className="mt-3 max-w-[24rem] space-y-2">
          {ui.bodyLines.map((line, i) => (
            <p key={i} className={cn("text-[15px] font-normal leading-relaxed", BODY_GRAY)}>
              {line}
              {ui.showBorrowDelayLearnMore &&
              ui.membershipLabel &&
              i === ui.bodyLines.length - 1 ? (
                <>
                  {" "}
                  <EmpruntBorrowDelayLearnMore
                    membershipLabel={ui.membershipLabel as SegnaBorrowMembershipLabel}
                  />
                </>
              ) : null}
            </p>
