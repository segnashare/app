...
export function ProfileAccountSettings({
  ...
  const facturesHref = `/profile/settings/factures?${profileSubflowTabQuery}`;
  const abonnementHref = `/profile/settings/abonnement?${profileSubflowTabQuery}`;
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col space-y-[4.5px] pt-[4.5px] pb-[max(2rem,calc(env(safe-area-inset-bottom,0px)+1.25rem))]">
        ...
        <SectionBlock title="Sécurité" ariaLabel="Sécurité">
          ...
          <SettingsLinkRow
            href={`/profile/password?${profileSubflowTabQuery}`}
            title="Mot de passe"
            subtitle="Modifie le mot de passe de ton compte."
          />
          <SettingsLinkRow
        ...
        <SectionBlock title="Abonnement" ariaLabel="Abonnement">
          {isSubscriber ? (
            <>
              <SettingsLinkRow href={facturesHref} title="Voir mes factures" subtitle="Historique et prochain paiement." />
              <SettingsLinkRow
                href={abonnementHref}
                title="Gérer mon abonnement"
                subtitle="Changer d’offre ou résilier."
              />
            </>
          ) : (
            <>
