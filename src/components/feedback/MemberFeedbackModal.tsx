...
import { MEMBER_FEEDBACK_OPEN_EVENT } from "@/lib/feedback/open-member-feedback-modal";
...
/** Modale signalement / aide — montée globalement, ouverte via bulle ou liens « Besoin d'aide ? ». */
export function MemberFeedbackModal() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(MEMBER_FEEDBACK_OPEN_EVENT, onOpen);
    return () => window.removeEventListener(MEMBER_FEEDBACK_OPEN_EVENT, onOpen);
  }, []);
  ...
  return createPortal(
    ...
    >
      ...
      >
        <SegnaDialogDismissButton onClick={close} className={busy ? "pointer-events-none opacity-40" : undefined} />
        ...
        {sent ? (
          ...
          <form className="mt-3" onSubmit={(e) => void handleSubmit(e)}>
            ...
            <label
              htmlFor={detailsId}
              className={cn(segnaDialogMontserrat.className, "mt-4 block text-[13px] font-semibold text-zinc-900")}
            >
