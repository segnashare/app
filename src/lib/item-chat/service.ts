...
export async function appendVisitorMessage(params: {
  ...
  // n8n → Discord (ne bloque pas l’UX si le webhook échoue)
...
export async function appendStaffMessage(params: {
  ...
  if (msgError) {
    // Idempotence si external_id déjà vu
  ...
  const message = toMessageDto(asMsg(msgData));
  const nextConversation = convData ? asConv(convData) : conversation;
  void import("@/lib/item-chat/notify-item-chat-staff-push").then(({ notifyItemChatStaffMessagePush }) =>
    notifyItemChatStaffMessagePush({
      admin,
      userId: nextConversation.user_id,
      conversationId: nextConversation.id,
      messageId: message.id,
      body: message.body,
      staffDisplayName: staffDisplayName ?? message.staffDisplayName,
    }),
  );
...
export async function recordUsefulnessRating(params: {
  ...
  const { ITEM_CHAT_USEFULNESS_THANKS_BODY } = await import(
    "@/lib/item-chat/lifecycle-constants"
  );
