...
export default function OnboardingCheckpointTwoPage() {
  ...
  const handleContinue = async () => {
    ...
    const { error } = await supabase.rpc("save_onboarding_progress", {
      p_current_step: "/onboarding/motivation",
      p_progress: { checkpoint: "/onboarding/2" },
    });
  ...
  return (
    <AppViewport className="bg-[#f9f9f8] px-0 py-0 md:w-[466px] md:!max-w-[466px] md:py-0">
      <OnboardingStepTracker currentStep="/onboarding/2" />
      <section className="px-6 pt-[clamp(2.75rem,10vh,7rem)] md:px-3 md:pt-30">
      ...
      >
        <p className={`${montserrat.className} text-[23px] font-bold leading-[1.15] text-white`}>Partage tes préférences</p>
      </button>
