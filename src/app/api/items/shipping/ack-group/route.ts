...
import { runIntakeShippingAckGrouped } from "@/lib/items/intake-cart-return-piggyback";
...
export async function POST(request: Request) {
  ...
  const res = await runIntakeShippingAckGrouped(admin, { userId: user.id, itemIds });
  if (!res.ok) {
    return NextResponse.json({ ok: false as const, error: res.error }, { status: res.status });
  }
  return NextResponse.json({ ok: true as const, item_ids: res.item_ids });
}
