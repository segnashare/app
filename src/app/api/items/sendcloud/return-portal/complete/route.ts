...
import { runMemberIntakeReturnPortalComplete } from "@/lib/items/member-intake-return-portal";
...
export async function POST(request: Request) {
  ...
  const servicePointId =
    typeof o.service_point_id === "number"
      ? o.service_point_id
      : parseInt(String(o.sendcloud_service_point_id ?? o.servicePointId ?? ""), 10);
  ...
  const res = await runMemberIntakeReturnPortalComplete(admin, {
    userId: user.id,
    itemIds,
    servicePointId,
  });
