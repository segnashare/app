...
function ProfileInsightsPromptsPageContent() {
  ...
  const buildInsightsUrl = (nextValues?: { p0?: string; p1?: string; p2?: string; r0?: string; r1?: string; r2?: string }) => {
  ...
  const goToTab = (tab: AnswersTabId) => {
    ...
    query.set("tab", tab);
    router.push(`/profile/insights/prompts?${query.toString()}`);
  };
  ...
  const selectPrompt = (prompt: string) => {
    ...
    router.push(buildInsightsUrl(next));
  };
    ...
    <AppViewport className="bg-[#f9f9f8] px-0 py-0 md:w-[min(92vw,760px)] md:!max-w-[760px] md:py-0">
      <div className="px-5 pb-4 pt-4 md:px-8">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button type="button" onClick={() => router.push(buildInsightsUrl())} className="text-zinc-900" aria-label="Fermer">
        ...
        <div
          ...
          onPointerMove={(event) => {
            ...
            if (!state.active || !tabsScrollRef.current) return;
            if (!event.currentTarget.hasPointerCapture(event.pointerId)) {
              event.currentTarget.setPointerCapture(event.pointerId);
            }
            const delta = event.clientX - state.lastX;
            tabsScrollRef.current.scrollLeft -= delta;
            dragStateRef.current = { ...state, lastX: event.clientX };
          }}
          onPointerUp={(event) => {
            dragStateRef.current = { active: false, lastX: 0 };
...
export default function ProfileInsightsPromptsPage() {
  return (
    <Suspense fallback={null}>
      <ProfileInsightsPromptsPageContent />
    </Suspense>
  );
}
